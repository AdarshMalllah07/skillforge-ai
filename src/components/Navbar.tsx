...
import { AppTab, TAB_PATH, pathToTab } from '../lib/routes';
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 gap-2">
            ...
            <nav className="hidden lg:flex items-center space-x-1.5 py-1">
              ...
              {role === 'INSTRUCTOR' && (
                <button
                  onClick={() => navigate('instructor_dashboard')}
                  className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-xl text-xs font-bold transition-all whitespace-nowrap ${
              ...
              {role === 'EVALUATOR' && (
                <button
                  onClick={() => navigate('evaluator_dashboard')}
                  className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-xl text-xs font-bold transition-all whitespace-nowrap ${
                    ...
                    {isAuthenticated && currentUser ? (
                      <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-3">
                        <div className="flex items-center justify-between">
                          ...
                          <span
                            className={`px-2 py-0.5 rounded border text-[10px] font-extrabold ${roleColors[currentUser.role]}`}
                          >
                            {currentUser.role}
                          </span>
                        </div>
